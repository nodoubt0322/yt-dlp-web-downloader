...
使用受保護 API 操作前，先在 UI token 欄位輸入 `dev-token`。
...
如果使用 Cloudflare Tunnel 或 reverse proxy，`PUBLIC_BASE_URL` 可讓產出的 `downloadUrl` 使用外部網址；本機開發可省略，前端會使用相對路徑。
...
建議使用 Cloudflare Access 作為外層保護，並保留 `ADMIN_TOKEN` 作為 app-level 防護。
